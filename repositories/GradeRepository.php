...
class GradeRepository extends Model implements IRepository {
    ...
    public function getTeacherGrades(int $teacherId, int $semester, string $academicYear): array {
        $sql = "SELECT g.*, st.first_name AS student_first, st.last_name AS student_last,
                ...
                ORDER BY s.subject_name, st.last_name";
    ...
    public function getGeneralAverage(int $studentId, int $semester, string $academicYear): float {
        $grades = $this->getStudentGrades($studentId, $semester, $academicYear);
        if (empty($grades)) return 0;
        $sum = array_sum(array_column($grades, 'quarterly_grade'));
        return round($sum / count($grades), 2);
    }

    public function getSemesters(): array {
        return $this->rawQuery(
            "SELECT DISTINCT semester, academic_year FROM grades ORDER BY academic_year DESC, semester DESC"
        );
    }
